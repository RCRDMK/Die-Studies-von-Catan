...
public class GameService extends AbstractService {
    ...
    public void onEndTurnRequest(EndTurnRequest request) {
        ...
        var game = gameManagement.getGame(request.getName()).get();
        ...
        if (inventory.getVictoryPoints() >= 10) {
            sendToAllInGame(game.getName(), new GameFinishedMessage(gameManagement.getGame(request.getName()).get().getName()));
